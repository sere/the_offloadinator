...
struct Producer {
  private:
    ...
    void function() { std::cout << "CPU task executing" << std::endl; }
    ...
    bool operator()() {
        if (offloadable_) {
            ...
            MPI_Send((void *)name_.c_str(), name_.size() + 1, MPI_CHAR, gpu_pe,
                     WORKTAG + id_, MPI_COMM_WORLD);
            std::cout << std::string(2 * (id_ + 1), '\t') << "MST " << id_
                      << ": Sent " << name_ << " to node " << gpu_pe << " tag "
                      << WORKTAG + id_ << std::endl;

            MPI_Status status;
            MPI_Recv(0, 0, MPI_INT, gpu_pe, WORKTAG + id_, MPI_COMM_WORLD,
                     &status);
            std::cout << std::string(2 * (id_ + 1), '\t') << "MST " << id_
...
struct Consumer {
  ...
  public:
    ...
    void operator()() {
        ...
        while (id_ == 0) {
            std::cout << std::string(mpi_id_, '\t') << "SLAVE " << mpi_id_
                      << " probing for work " << std::endl;
            MPI_Mprobe(MPI_MASTER_THREAD, MPI_ANY_TAG, MPI_COMM_WORLD, &message,
                       &status);
            std::cout << std::string(mpi_id_, '\t') << "SLAVE " << mpi_id_
                      << " probed tag " << status.MPI_TAG << ", node "
            ...
            MPI_Mrecv(static_cast<void *>(rec_buf), count, MPI_CHAR, &message,
                      &status);
            std::cout << std::string(mpi_id_, '\t') << "SLAVE " << mpi_id_
                      << " received work " << rec_buf << " from "
                      << status.MPI_SOURCE << std::endl;
            MPI_Send(0, 0, MPI_INT, status.MPI_SOURCE, status.MPI_TAG,
                     MPI_COMM_WORLD);
            std::cout << std::string(mpi_id_, '\t') << "SLAVE " << mpi_id_
                      << " sent answer to node " << status.MPI_SOURCE << " tag "
